...
public class SpawnAreaHandler {
    ...
    public static void loadAreas() throws IOException, ObjectMappingException {
        ...
        for (Map.Entry<String, Area> areaMap : AreaHandler.areaMap.entrySet()) {

            String areaName = areaMap.getKey();
            SpawnManager.logger.info("Creating spawn files for area: " + areaName);
            Area area = areaMap.getValue();
            Path dir = ConfigUtils.checkDir(Paths.get("./config/spawnmanager/areas/" + areaName));
